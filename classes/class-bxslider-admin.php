    ...
    class Bxslider_Admin {
        ...
        public function register_admin_scripts() {
            ...
            if('bxslider' == get_post_type()){ // Load only scripts here and not on all admin pages
                ...
                wp_enqueue_style( 'bxslider-admin-styles', BXSLIDER_URL.'css/admin.css', array(), BXSLIDER_VERSION  );
                
                // Scripts
                wp_dequeue_script( 'autosave' );//disable autosave
                
                wp_enqueue_script( 'store', BXSLIDER_URL.'js/store-json2.min.js', array('jquery'), BXSLIDER_VERSION );
                
                wp_register_script( 'bxslider-admin-script', BXSLIDER_URL.'js/admin.js', array('jquery', 'jquery-ui-sortable'), BXSLIDER_VERSION  );
        ...
        public function render_slides_meta_box( $post ){
            ...
            $slides = Bxslider_Data::get_slides($post->ID);
        ...
        public function render_general_options_meta_box( $post ){
            ...
            if($options['use_css']=='true'){
                $vars['easing_options'] = $this->to_select_options(Bxslider_Data::get_css_easing_options(), $options['easing']);
            } else {
                $vars['easing_options'] = $this->to_select_options(Bxslider_Data::get_jquery_easing_options(), $options['easing']);
            }
        ...
        public function custom_column( $column_name, $post_id ){
            if ($column_name == 'images') {
                echo '<div style="text-align:center; max-width:40px;">' .Bxslider_Data::get_slides_count($post_id). '</div>';
            }
            if ($column_name == 'id') {
        ...
        public function admin_footer() {
            ...
            if(get_post_type()=='bxslider'){
                ...
                $slide = Bxslider_Data::get_slide_defaults();
