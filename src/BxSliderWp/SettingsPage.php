...
*/
class BxSliderWp_SettingsPage extends BxSliderWp_Base {
	...
	*/
	public function render_settings_page($post){
		...
		$vars['debug_info'] = ($this->plugin['debug']) ? '<pre>'.print_r( $vars['settings_data'], true ).'</pre>' : '';
	...
	*/
	public function validate_options( $input ) {
		...
		if( isset($_POST['reset']) ){
			...
			add_settings_error( $this->plugin['settings_page.menu_slug'], 'restore_defaults', __( 'Default options restored.', $this->plugin['textdomain']), 'updated fade' );
		}
		return $input;
	}
